angular.module('myApp')
    .controller('Dm003_OrderMainInfo', function ($scope, netRequest, dialog, sysMessage,$compile) {
        ...
        $scope.config = {
            colModel: [
                        { label: "商户名称", name: "dmNmae", width: "14%" },
                        { label: "商品名称", name: "commodityName", width: "15%" },
                        { label: "小区名称", name: "comName", width: "15%" },
                        { label: "用户名称", name: "umName", width: "15%" },
                        { label: "订单价格", name: "orderprice", width: "15%" },
                        { label: "订单状态", name: "stateName", width: "25%" },
        ...
        $scope.onNew = function () {
            $scope.$broadcast("showDm003_OrderMainEdit", "new", {}, $scope.more,$scope.condition.selDmName,$scope.condition.selDMcommodity);
        }

        $scope.onEdit = function (item) {
            $scope.$broadcast("showDm003_OrderMainEdit", "edit", item, $scope.more,$scope.condition.selDmName,$scope.condition.selDMcommodity);
        }
