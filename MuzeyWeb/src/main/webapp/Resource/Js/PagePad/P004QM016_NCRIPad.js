angular.module('myApp')
    .controller('P004QM016_NCRIPadCtrl', function ($scope, netRequest, dialog) {
        ...
        $scope.config = {
            colModel: [
                         { label: "NCR序号", name: "NCRNo", width: "80px" },
                         { label: "NCR分类", name: "NCRTypeName", width: "150px" },
                         { label: "工单号", name: "JobNo", width: "80px" },
                         { label: "加工序列号", name: "SerialNo", width: "100px" },
                         { label: "物料号", name: "MaterialCode", width: "100px" },
                         { label: "产品种类", name: "AssemblyDesc", width: "80px" },
                         { label: "缺陷数量", name: "Amounts", width: "80px" },
                         { label: "创建时间", name: "CreateTime", width: "120px" },
                         { label: "状态", name: "Status", width: "40px",
                            format: [
                                     { value: "0", display: "开启" },
                                     { value: "1", display: "关闭" }
                                    ]
                         },
                          {
                              ...
                              button: [{
                                  caption: '>', action: function (item) {
                                      $scope.$broadcast("showP004QM0016_NCRFlowIPad", item);
    ...
    .directive('ncrflowipadEdit', function (netRequest, dialog) {
        return {
            ...
            templateUrl: 'View/P099Pad/P004QM0016_NCRFlowIPad.html?v=' + Math.random(),
            link: function ($scope, iElm, iAttrs, controller) {
                $scope.$on("showP004QM0016_NCRFlowIPad", function (event, ncrEvent) {
